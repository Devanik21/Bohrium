...
reproductive_rate_base = population_growth_rate{organism.name} went extinct")
    ...
    if random.random() < pandemic_frequency * 0.01 and len(new_organisms) > 0:
        ...
        # CONTINUATION - Place this RIGHT AFTER the incomplete line:
# events.append(f"

# Complete the pandemic event and continue all remaining code:
        events.append(f"🦠 PANDEMIC struck {victim.name}!")
    
    if random.random() < supervolcano_eruptions * 0.01:
        events.append("🌋 SUPERVOLCANO ERUPTION! Climate shift!")
        environment.temperature += random.uniform(-50, -20)
        for org in new_organisms:
            org.environment_fitness *= 0.5
    
    if random.random() < gamma_ray_burst * 0.001:
        events.append("💥 GAMMA RAY BURST! Atmospheric damage!")
        environment.radiation_level *= 2.0
        for org in new_organisms:
            if org.radiation_resistance < 0.5:
                org.population = int(org.population * 0.1)
    
    # Ice age events
    if random.random() < ice_age_frequency * 0.01:
        events.append("❄️ ICE AGE beginning!")
        environment.temperature -= random.uniform(10, 30)
    
    # Solar events
    if random.random() < solar_flare_intensity * 0.05:
        events.append("☀️ SOLAR FLARE hit the planet!")
        environment.radiation_level += random.uniform(0.1, 0.5)
    
    # Technological disasters
    advanced_civs = [o for o in new_organisms if o.technological_level > 5.0]
    if advanced_civs:
        if random.random() < nuclear_war_probability * 0.01:
            victim = random.choice(advanced_civs)
            events.append(f"☢️ NUCLEAR WAR devastated {victim.name}!")
            victim.population = int(victim.population * 0.05)
            victim.technological_level *= 0.5
            environment.radiation_level += 1.0
        
        if random.random() < ai_takeover_risk * 0.01:
            victim = random.choice(advanced_civs)
            events.append(f"🤖 AI SINGULARITY in {victim.name} civilization!")
            victim.life_type = LifeType.MACHINE
            victim.intelligence *= 3.0
            victim.technological_level *= 2.0
        
        if random.random() < grey_goo_scenario * 0.01:
            victim = random.choice(advanced_civs)
            events.append(f"⚫ GREY GOO scenario in {victim.name}!")
            victim.population = int(victim.population * 0.01)
    
    # Positive events
    if random.random() < adaptive_radiation * 0.02 and len(new_organisms) > 0:
        lucky = random.choice(new_organisms)
        events.append(f"🌟 ADAPTIVE RADIATION burst in {lucky.name}!")
        lucky.adaptability *= 1.5
        lucky.population = int(lucky.population * 1.5)
    
    # Symbiosis events
    if len(new_organisms) >= 2 and random.random() < symbiosis_frequency * 0.05:
        org1, org2 = random.sample(new_organisms, 2)
        events.append(f"🤝 SYMBIOSIS formed between {org1.name} and {org2.name}!")
        org1.cooperation += 0.2
        org2.cooperation += 0.2
        org1.environment_fitness *= 1.3
        org2.environment_fitness *= 1.3
    
    # Evolutionary leaps
    for org in new_organisms:
        if org.complexity.value >= ComplexityLevel.INTELLIGENT.value and random.random() < 0.001:
            if org.consciousness_level > 0.8 and org.technological_level > 10.0:
                if org.complexity.value < ComplexityLevel.TRANSCENDENT.value:
                    org.complexity = ComplexityLevel.TRANSCENDENT
                    events.append(f"✨ {org.name} achieved TRANSCENDENCE!")
        
        if org.complexity.value >= ComplexityLevel.TRANSCENDENT.value and random.random() < 0.0001:
            if org.dimensional_awareness > 0.5:
                if org.complexity.value < ComplexityLevel.COSMIC.value:
                    org.complexity = ComplexityLevel.COSMIC
                    events.append(f"🌌 {org.name} reached COSMIC consciousness!")
        
        if org.complexity.value >= ComplexityLevel.COSMIC.value and random.random() < 0.00001:
            if org.intelligence > 50.0 and org.technological_level > 50.0:
                if org.complexity.value < ComplexityLevel.OMNIPOTENT.value:
                    org.complexity = ComplexityLevel.OMNIPOTENT
                    events.append(f"♾️ {org.name} achieved OMNIPOTENCE! Universe-shaping power!")
    
    # Environmental recovery
    if environment.temperature < base_temperature - 50:
        environment.temperature += random.uniform(1, 5)
    if environment.temperature > base_temperature + 50:
        environment.temperature -= random.uniform(1, 5)
    if environment.radiation_level > cosmic_radiation * 2:
        environment.radiation_level *= 0.95
    
    # Population pressure effects
    total_pop = sum(o.population for o in new_organisms)
    if total_pop > carrying_capacity * 1.5:
        events.append("⚠️ OVERPOPULATION crisis! Resources depleted!")
        for org in new_organisms:
            org.population = int(org.population * 0.7)
    
    # Competition dynamics
    if len(new_organisms) > 1:
        sorted_orgs = sorted(new_organisms, key=lambda x: x.environment_fitness * x.population, reverse=True)
        
        if sorted_orgs[0].population > total_pop * 0.4:
            if random.random() < 0.1:
                events.append(f"👑 {sorted_orgs[0].name} became the DOMINANT SPECIES!")
        
        if random.random() < competition_intensity * 0.1:
            if len(new_organisms) > 5:
                weakest = sorted_orgs[-1]
                if weakest.population < 50:
                    events.append(f"💀 {weakest.name} outcompeted to extinction!")
                    new_organisms.remove(weakest)
    
    # Cooperation benefits
    cooperative_orgs = [o for o in new_organisms if o.cooperation > 0.7]
    if len(cooperative_orgs) >= 2 and random.random() < cooperation_benefit * 0.05:
        for org in cooperative_orgs:
            org.population = int(org.population * 1.1)
            org.intelligence *= 1.05
        events.append(f"🤝 Cooperative species thriving! {len(cooperative_orgs)} species benefit!")
    
    # Parasitism effects
    if len(new_organisms) >= 2 and random.random() < parasitism_rate * 0.1:
        host = random.choice(new_organisms)
        events.append(f"🦠 Parasitic outbreak affects {host.name}!")
        host.population = int(host.population * 0.8)
        host.energy *= 0.9
    
    # Predation dynamics
    predators = [o for o in new_organisms if o.aggression > 0.6]
    prey = [o for o in new_organisms if o.aggression < 0.4]
    
    if predators and prey and random.random() < predation_pressure * 0.1:
        predator = random.choice(predators)
        victim = random.choice(prey)
        events.append(f"🦁 {predator.name} hunts {victim.name}!")
        victim.population = int(victim.population * 0.85)
        predator.energy += 20
        predator.population = int(predator.population * 1.1)
    
    # Dimensional breaches
    dimension_aware = [o for o in new_organisms if o.dimensional_awareness > 0.5]
    if dimension_aware and random.random() < dimensional_travel * 0.01:
        breacher = random.choice(dimension_aware)
        events.append(f"🌀 {breacher.name} opened a DIMENSIONAL PORTAL!")
        breacher.technological_level += 5.0
        breacher.intelligence += 2.0
    
    # Technological artifacts
    tech_advanced = [o for o in new_organisms if o.technological_level > 3.0]
    if tech_advanced and random.random() < innovation_rate * 0.1:
        innovator = random.choice(tech_advanced)
        events.append(f"🔬 {innovator.name} created a revolutionary artifact!")
        innovator.technological_level += random.uniform(0.5, 2.0)
    
    # Quantum phenomena
    quantum_entities = [o for o in new_organisms if o.quantum_coherence > 0.5]
    if quantum_entities and random.random() < quantum_tunneling * 0.05:
        q_org = random.choice(quantum_entities)
        events.append(f"⚛️ {q_org.name} achieved quantum tunneling capabilities!")
        q_org.dimensional_awareness += 0.2
    
    # Energy being emergence
    energy_candidates = [o for o in new_organisms if o.intelligence > 10.0 and o.technological_level > 15.0]
    if energy_candidates and random.random() < energy_being_emergence * 0.01:
        transformer = random.choice(energy_candidates)
        events.append(f"⚡ {transformer.name} transcended to PURE ENERGY form!")
        transformer.life_type = LifeType.ENERGY_BEING
        transformer.energy_efficiency = 0.99
        transformer.mass = 0.0
    
    # Collective consciousness formation
    psionic_orgs = [o for o in new_organisms if o.telepathic_ability > 0.5]
    if len(psionic_orgs) >= 3 and random.random() < collective_consciousness * 0.02:
        events.append(f"🧠 COLLECTIVE CONSCIOUSNESS formed among {len(psionic_orgs)} species!")
        for org in psionic_orgs:
            org.intelligence *= 1.5
            org.telepathic_ability = 1.0
            org.consciousness_level = min(1.0, org.consciousness_level * 1.3)
    
    # Reality warping events
    cosmic_beings = [o for o in new_organisms if o.complexity.value >= ComplexityLevel.COSMIC.value]
    if cosmic_beings and random.random() < reality_warping * 0.01:
        warper = random.choice(cosmic_beings)
        events.append(f"🎭 {warper.name} WARPED REALITY! Fundamental changes occurring!")
        environment.temperature = random.uniform(200, 400)
        environment.gravity = random.uniform(0.5, 2.0)
        warper.technological_level = 100.0
    
    # Universe creation attempts
    omnipotent_beings = [o for o in new_organisms if o.complexity.value >= ComplexityLevel.OMNIPOTENT.value]
    if omnipotent_beings and random.random() < universe_creation * 0.001:
        creator = random.choice(omnipotent_beings)
        events.append(f"🌌 {creator.name} attempted to CREATE A NEW UNIVERSE!")
        creator.dimensional_awareness = 1.0
    
    # Ensure organisms survive if list becomes empty
    if not new_organisms and len(organisms) > 0:
        survivor = random.choice(organisms)
        survivor.population = 100
        new_organisms = [survivor]
        events.append(f"🌱 {survivor.name} survived and recolonized!")
    
    # If completely empty, generate new life
    if not new_organisms:
        new_organisms = [create_primordial_organism(9999, params)]
        events.append("🌟 ABIOGENESIS! New life spontaneously emerged!")
    ...
    # Competition dynamics
    if len(new_organisms) > 1:
        # Sort by fitness and population
        sorted_orgs = sorted(new_organisms, key=lambda x: x.environment_fitness * x.population, reverse=True)
    ...
    # Dimensional breaches (for advanced civilizations)
    dimension_aware = [o for o in new_organisms if o.dimensional_awareness > 0.5]
    if dimension_aware and random.random() < dimensional_travel * 0.01:
        breacher = random.choice(dimension_aware)
        events.append(f"🌀 {breacher.name} opened a DIMENSIONAL PORTAL!")
...
# Add this at the very end of the file to ensure everything runs
if __name__ == "__main__":
    st.sidebar.markdown("---")
    st.sidebar.markdown("### 📖 Instructions")
    st.sidebar.info("""
    1. Click '🌟 Initialize Universe' to create your universe
    2. Adjust physics, chemistry, and biology parameters in the sidebar
    3. Press '▶️ Start' to begin evolution
    4. Watch organisms evolve, adapt, and transcend!
    5. Explore different tabs to analyze your universe
    
    **Tips:**
    - Increase mutation rate for faster evolution
    - Higher complexity tendency leads to intelligent life
    - Extreme physics can create exotic life forms
    - Enable Advanced Mode for more controls
    """)
    
    st.sidebar.markdown("---")
    st.sidebar.markdown("""
    <div style='text-align: center; font-size: 0.8em; color: #666;'>
    <p>🔬 Universe Sandbox AI v2.0</p>
    <p>A comprehensive evolution simulator</p>
    <p>Explore infinite possibilities of life!</p>
    </div>
    """, unsafe_allow_html=True)
